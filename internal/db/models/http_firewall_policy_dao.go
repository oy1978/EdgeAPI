...
func (this *HTTPFirewallPolicyDAO) ComposeFirewallPolicy(tx *dbs.Tx, policyId int64, forNode bool, cacheMap *utils.CacheMap) (*firewallconfigs.HTTPFirewallPolicy, error) {
	...
	if IsNotNull(policy.SilOptions) {
		var SilAction = &firewallconfigs.HTTPFirewallSilAction{}
		err = json.Unmarshal(policy.SilOptions, SilAction)
		if err != nil {
			return config, err
		}
		config.SilOptions = SilAction
	}
	// syn flood
